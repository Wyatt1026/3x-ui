...
xui_repo_url="https://github.com/${xui_repo}"
xui_api_releases_url="https://api.github.com/repos/${xui_repo}/releases"
...
ensure_acme_ca_account() {
    ...
    if [[ "${acme_ca}" == "zerossl" ]]; then
        ...
        if [[ -n "${XUI_ACME_EMAIL:-}" ]]; then
            account_email="${XUI_ACME_EMAIL// /}"
        fi
        if [[ -z "${account_email}" && "$NONINTERACTIVE" == "1" ]]; then
            echo -e "${red}ZeroSSL requires XUI_ACME_EMAIL in non-interactive mode.${plain}"
            return 1
        fi
        while [[ -z "${account_email}" ]]; do
            if [[ "$NONINTERACTIVE" != "1" ]]; then
                read -rp "Please enter your email for ZeroSSL account registration: " account_email
                account_email="${account_email// /}"
            fi
            [[ "${account_email}" =~ ^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$ ]] && break
        ...
        if [ $? -ne 0 ]; then
            ...
            local eab_kid=""
            local eab_hmac_key=""
            eab_kid="${XUI_ACME_EAB_KID:-}"
            eab_hmac_key="${XUI_ACME_EAB_HMAC_KEY:-}"
            if [[ -z "${eab_kid}" && "$NONINTERACTIVE" != "1" ]]; then
                read -rp "Enter ZeroSSL EAB KID (leave empty to abort): " eab_kid
            fi
            if [[ -z "${eab_kid}" ]]; then
                return 1
            fi
            if [[ -z "${eab_hmac_key}" && "$NONINTERACTIVE" != "1" ]]; then
    ...
    elif [[ -n "${XUI_ACME_EMAIL:-}" ]]; then
        ~/.acme.sh/acme.sh --register-account -m "${XUI_ACME_EMAIL}" --server "${acme_ca}" > /dev/null 2>&1 || true
    fi
...
setup_ip_certificate() {
    ...
    # Issue certificate with the selected CA.
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "${xui_api_releases_url}/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 60 "${xui_api_releases_url}/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLR --retry 5 --retry-delay 3 --connect-timeout 15 --max-time 300 -o ${xui_folder}-linux-$(arch).tar.gz ${xui_repo_url}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
