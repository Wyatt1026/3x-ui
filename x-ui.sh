...
update_dev() {
    ...
    XUI_REPO="${xui_repo}" XUI_BRANCH="${xui_branch}" XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls "${xui_raw_url}/update.sh")
    if [[ $? == 0 ]]; then
...
│  ${green}21.${plain} Cloudflare SSL Certificate (choose CA)   │
